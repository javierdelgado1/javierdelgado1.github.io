 ...
 * configuración del sitio, idiomas, lectura de posts/<lang>/*.md, front matter, slugs, URLs y limpieza de texto.
 */
...
// ───────────────────────────── idiomas ─────────────────────────────
// site.languages: [{ code, label, locale, basePath, default }]
// El idioma "default" sirve también sin prefijo en la raíz (/posts/x/); los demás van bajo su basePath (/es/posts/x/).
export const LANGS = site.languages || [{ code: "en", label: "English", locale: "en_US", basePath: "", default: true }];
...
/** Ruta relativa al dominio con el basePath del idioma delante: href("es", "/posts/x/") → "/es/posts/x/". */
export const href = (langCode, p) => {
  if (isExternal(p)) return p;
  const lang = langByCode[langCode] || DEFAULT_LANG;
  return lang.basePath + p;
};

/** URL absoluta en un idioma dado. */
export const absUrl = (langCode, p) => (isExternal(p) ? p : site.baseUrl + href(langCode, p));

/** URL absoluta sin idioma (recursos compartidos: /assets/..., /favicon...). No lleva basePath de idioma. */
export const absUrlShared = (p) => (isExternal(p) ? p : site.baseUrl + p);
...
/** Lee posts/<langCode>/*.md y devuelve los publicados (sin draft), sin renderizar. */
export function loadPostSources(langCode, { includeDrafts = false } = {}) {
  ...
  for (const file of fs.readdirSync(dir).filter((f) => f.endsWith(".md"))) {
    ...
    if (!meta.title) {
      console.warn(`⚠  ${langCode}/${file}: falta "title" en el front matter, se omite.`);
      continue;
    }
    if (!meta.translationKey) {
      console.warn(`⚠  ${langCode}/${file}: falta "translationKey" en el front matter, se omite.`);
      continue;
    }
